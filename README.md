...
*Many times ideas come in the worst moment and they fade away before you have the time to explore them better. The objective of this _project is developing a system capable of giving tips and opinions in quasi-real-time about anything you ask. The ultimate assistant will be able to be accessed from any authorized microphone inside your house or your phone, it should run constantly in the background and when summoned should be able to generate meaningful answers (with a badass voice) as well as interface with the pc or a server and save/read/write files that can be accessed later. In addition, it might interface with some external gadgets (IoT) but that's extra.*
...
### Enviroment
1. Make a new, empty venv;
2. ```pip install requirements.txt```; Warning: this file is faulty, wait for an update <div><span style="color:grey"><i>because I moved a virtual environment, that caused references to mess up and so the requirements.txt were generated with the wrong pip... dumb me, do not ever move a venv :)</i> </span></div>
3. install [TTS](https://github.com/coqui-ai/tts) (once the setup.py is done, delete everything besides the venv);
4. download the material of this repo;
<br>
...
- Get the OpenAI API from their official website. This will allow us to send and receive material to Whisper and ChatGPT. 
...
- The `say()` function will perform voice duplication to speak with Jarvis/Someone's voice; if the argument is not in English, IBM Watson will send the response from one of their nice text-to-speech models. If everything fails, the fucntions will rely on pyttsx3 which is a fast yet not as cool alternative;
<p align="center">
